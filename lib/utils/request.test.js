...
tap.test('WarehouseRequest', async (t) => {
  ...
  t.test('stores agent from constructor', async (t) => {
    ...
    t.equal(req._agent, agent);
  });

  t.test('uses agent on get requests', async (t) => {
    nock(BASE_URL).get('/objects/foo').reply(200, { ok: true });
    const https = require('https');
    const agent = new https.Agent({ keepAlive: true });
    const req = new WarehouseRequest({
      baseUrl: BASE_URL,
  ...
  t.test('uses agent on post requests', async (t) => {
    nock(BASE_URL).post('/objects', { name: 'bar' }).reply(200, { ok: true });
    const https = require('https');
    const agent = new https.Agent({ keepAlive: true });
    const req = new WarehouseRequest({
      baseUrl: BASE_URL,
    ...
    const result = await req.post('/objects', { name: 'bar' });
    t.same(result, { ok: true });
  });
});
