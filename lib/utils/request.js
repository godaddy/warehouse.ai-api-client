 ...
 * @property {import('http').Agent} [agent] Optional HTTP agent (e.g. keep-alive agent)
 */
...
class WarehouseRequest {
   ...
   */
  constructor({ baseUrl, username, password, agent }) {
  ...
  async _reqWithJSONBody({ endpoint, body, method }) {
    ...
    const resp = await fetch(`${this._baseUrl}${endpoint}`, {
      ...
      method,
      ...(this._agent !== undefined && { agent: this._agent })
    });
